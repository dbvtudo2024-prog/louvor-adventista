...
export function getSupabase() {
  ...
  // Credenciais padrão do Supabase configuradas para o projeto do usuário
  const DEFAULT_SUPABASE_URL = "https://xdwplwqpnsglaitedehu.supabase.co";
  const DEFAULT_SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inhkd3Bsd3FwbnNnbGFpdGVkZWh1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzI3NjQ1OTEsImV4cCI6MjA4ODM0MDU5MX0.GupAMmWSDv39aeFwp0QfmYReClfGkz_DUsbAnGNADDY";

  const supabaseUrl = (envUrl || DEFAULT_SUPABASE_URL).trim();
  const supabaseAnonKey = (envKey || DEFAULT_SUPABASE_ANON_KEY).trim();
